...
def verify_segments(list_m, list_sd, dictionary_walk):
...
def tempo_de_ciclo(lista_tamanho_passo):
	p_v = picos_e_vales(lista_tamanho_passo)
	posicoes_picos = p_v['maxima_locations']
	lista_de_intervalos = []
	lista_negra = []

	anterior = 0
	proximo = 0
	for posicao_atual in posicoes_picos:
		proximo = posicao_atual
		intervalo = proximo - anterior
		lista_de_intervalos.append(intervalo)
		anterior = proximo

	mediaIntervaloPicos = media_a(lista_de_intervalos)
	dpIntervaloPicos = desvio_padrao(lista_de_intervalos)

	for diferenca in lista_de_intervalos:
		if diferenca > mediaIntervaloPicos + (2*dpIntervaloPicos):
			lista_negra.append(diferenca)
		if diferenca < mediaIntervaloPicos - (2*dpIntervaloPicos):
			lista_negra.append(diferenca)

	return media_a(list(set(lista_de_intervalos) - set(lista_negra)))

#def time_of_cycle(list_step_size):
def calculate_cycle_time(list_step_size):
	...
	for current_position in peeks_positions:
		_next = current_position
		_range = _next - before
		list_of_ranges.append(_range)
		before = _next
	...
	for difference in list_of_ranges:
		if difference > averagePeakRange + (2*sdPeakRange):
...
for line in files:
	...
	frame_lista_negra = verify_segments(list_average, list_sd_deviation, walk)
	...
	# tempo_ciclo = tempo_de_ciclo([float(n) for n in lista_tamanho_passo[0]]) / 30
	# velocidade = 2*media_a(max(lista_tamanho_passo)) / tempo_ciclo

	# fw.write(',' + str(media_a(max(lista_tamanho_passo))) + ',' + str(2*media_a(max(lista_tamanho_passo))) + ',' + str(media_a(max(list_height)))
	# 		  + ','  + str(velocidade) + ',' + str(tempo_ciclo))
	# fw.write('\n')

	# # fw2.write(individuo)
	# # for i in range(len(lista_mediana)):
	# # 	fw2.write(','+ str(lista_mediana[i]))
	# # fw2.write(',' + str(media_a(max(lista_tamanho_passo))) + ',' + str(2*media_a(max(lista_tamanho_passo))) + ',' + str(media_a(max(list_height)))
	# # 		     + str(2*media_a(max(lista_tamanho_passo)) / (tempo_de_ciclo(lista_tamanho_passo)/30)) + ',' + str(tempo_de_ciclo(lista_tamanho_passo)/30))
	# # fw2.write('\n')

	# fw3.write(individuo)
	# for i in range(len(list_step_size)):
	# 	fw3.write(',' + str(lista_step_size[i]))
	# fw3.write('\n')


 #"Cycle_time,", "Stride_size,", "Step_size,","velocity"
	cycle_time = calculate_cycle_time([float(n) for n in list_step_size[0]]) / 30
	velocity = 2*average_a(max(list_step_size)) / cycle_time

	fw.write(',' + str(average_a(max(list_height))))
	...
	# fw2.write(',' + str(median_a(max(list_step_size))) + ',' + str(2*median_a(max(list_step_size))) + ',' + str(median_a(max(list_height)))
	#  		     + str(2*median_a(max(list_step_size)) / (calculate_cycle_time(list_step_size)/30)) + ',' + str(calculate_cycle_time(list_step_size)/30))
	# fw2.write('\n')

	fw2.write(',' + str(median_a(max(list_height))))
	fw2.write(',' + str(cycle_time))
	fw2.write(',' + str(2*(median_a(max(list_step_size)))))
	fw2.write(',' + str(median_a(max(list_step_size))))
	fw2.write(',' + str(velocity))
	#fw.write(',' + gender[individual])
	fw2.write('\n')
